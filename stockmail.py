...
def main():
    ...
    def get_thumbnail(news_item, default_img):
        try:
    ...
    def display_news(stock_ticker):
        ...
        default_image_url = "https://i.ibb.co/Z24gZ2v/Black-White-Minimalist-Logo.jpg"  # Replace with your actual default image URL
        newscol1, newscol2 = st.columns([1, 1])
        for i, news_item in enumerate(articles):
            img_url = get_thumbnail(news_item, default_image_url)
            if img_url is None or not is_valid_url(img_url):
                img = Image.open(requests.get(default_image_url, stream=True).raw)
            else:
                try:
    ...
    # def display_news(stock_ticker):
    #     articles = get_stock_news(stock_ticker)[:5]  # Limit to first 5 articles
    #     newscol1, newscol2 = st.columns([1, 1])
